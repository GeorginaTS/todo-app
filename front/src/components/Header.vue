  ...
  <header class="bg-stone-200 flex flex-col justify-between items-center w-full h-18">
    <div class="flex justify-between w-full px-8 py-2">
        ...
        <nav class="w-fit flex gap-2 items-center mx-2" v-if="userStore.loggedIn">
          <RouterLink to="/auth/profile"><div class="w-10 h-10 flex justify-center items-center rounded-full div_primary text-white font-bold hover:brightness-150">{{userStore.user.name[0]}}</div></RouterLink>
        </nav> 
      </div>
      <hr />
    </header>
